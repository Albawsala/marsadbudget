...
var BubbleTree = function(config, onHover, onUnHover) {
	...
	me.traverse = function(node, index) {
		...
		var colorsl= ['#aed8e5' , '#c84c41' , '#FFA500' , "#0000ff" , "#fbd3c9" , '#aed8e5' , '#c84c41' , '#FFA500' , "#0000ff" , "#fbd3c9"];
		if (node.level > 0) {
			pc = node.parent.children;
			if (pc.length > 0) {
				node.left = pc[(index-1+pc.length) % pc.length];
				node.right = pc[(Number(index)+1) % pc.length];
				if (node.level == 1) node.color = vis4color.fromHex(colorsl[index] , .7, .5).x;
				if (node.level > 1) node.color = vis4color.fromHex(node.parent.color).lightness('*1.2').x;
			}
		}
...
BubbleTree.Bubbles.Plain = function(node, bubblechart, origin, radius, angle, color) {
	...
	me.draw = function() {
		...
		var w = Math.max(70, 3*r)+35;
		me.label2.css({ width: w+'px', opacity: me.alpha });
		me.label2.css({ left: (x - w*0.8)+'px', top: (y + r)+'px' });
